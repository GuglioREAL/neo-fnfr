...
lovefile:
	...
	@cd src/love; zip -r -9 ../../build/lovefile/neo-fnfr.love .

	@mkdir -p build/release
	@rm -f build/release/neo-fnfr-lovefile.zip
	@cd build/lovefile; zip -9 -r ../release/neo-fnfr-lovefile.zip .
...
win64: lovefile
	...
	@cat resources/win64/love/love.exe build/lovefile/funkin-rewritten.love > build/win64/neo-fnfr.exe

	@mkdir -p build/release
	@rm -f build/release/neo-fnfr-win64.zip
	@cd build/win64; zip -9 -r ../release/neo-fnfr-win64.zip .
...
win32: lovefile
	...
	@cat resources/win32/love/love.exe build/lovefile/neo-fnfr.love > build/win32/neo-fnfr.exe

	@mkdir -p build/release
	@rm -f build/release/neo-fnfr-win32.zip
	@cd build/win32; zip -9 -r ../release/neo-fnfr-win32.zip .

macos: lovefile
	@rm -rf build/macos
	@mkdir -p "build/macos/Neo FNFR.app"

	@cp -r resources/macos/love.app/. "build/macos/Neo FNFR.app"

	@cp build/lovefile/funkin-rewritten.love "build/macos/Neo FNFR.app/Contents/Resources"

	@mkdir -p build/release
	@rm -f build/release/neo-fnfr-macos.zip
	@cd build/macos; zip -9 -r ../release/neo-fnfr-macos.zip .

switch: lovefile
	@rm -rf build/switch
	@mkdir -p build/switch/switch/funkin-rewritten

	@nacptool --create "Friday Night Funkin' NEO" GameVasion "$(shell cat version.txt)" build/switch/funkin-rewritten.nacp

	@mkdir build/switch/romfs
	@cp build/lovefile/neo-fnfr.love build/switch/romfs/game.love
