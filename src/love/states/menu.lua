...
local logo = love.filesystem.load("sprites/menu/logoBumpin.lua")()

local menuLogo = love.filesystem.load("sprites/menu/neomenu.lua")()
...
local function switchMenu(menu)
	...
	elseif menu == 2 then
		...
		function drawFunc()
			graphics.setColor(1, 1, 0)
			if menuState == 4 then
				love.graphics.printf("Downscroll: True\nShow FPS: False\n")
			elseif menuState == 2 then
				if songDifficulty == 3 then
					love.graphics.printf("Choose a difficulty: < Hard >", -640, 285, 853, "center", nil, 1.5, 1.5)
				elseif songDifficulty == 2 then
...
return {
	...
	draw = function(self)
		love.graphics.push()
			...
			titleBG:draw()
			--if weekNum == 1 then
			--	weekLogo = love.filesystem.load("sprites/menu/neomenu.lua")()
			--	menuLogo:animate("tutorial", true)
			--	menuLogo:draw()
